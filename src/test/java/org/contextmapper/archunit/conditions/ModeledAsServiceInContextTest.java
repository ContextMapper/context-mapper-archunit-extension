...
class ModeledAsServiceInContextTest {
    ...
    @Test
    void check_WhenServiceIsModeled_ThenConditionIsFulfilled() {
        // given
        JavaClasses classes = new ClassFileImporter()
                .importPackages("org.contextmapper.archunit.sample.sampleaggregate1");

        // when, then
        serviceClassesShouldBeModeledInCml(sampleContext).check(classes);
    }

    @Test
    void check_WhenServiceIsNotModeled_ThenAddEvent() {
        // given
        JavaClasses classes = new ClassFileImporter()
                .importPackages("org.contextmapper.archunit.sample.sampleservice_notmodeled");
